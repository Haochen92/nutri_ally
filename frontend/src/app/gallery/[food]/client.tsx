...
interface FoodInfoClientProps {
  foodData: FoodItem;
}

export default function FoodInfoClient({ foodData }: FoodInfoClientProps) {
  const [opened, { toggle }] = useDisclosure(false);
  const dailyRecommendedIntake = getDailyRecommendedIntake();

  const mapNutrient = (nutrient: string): [string, string, string] => {
    let value: number, dailyValue: number;
    const unit = dailyRecommendedIntake[nutrient].unit;

    if (foodData[nutrient]) {
      value = (foodData[nutrient] as number) * dailyRecommendedIntake[nutrient].multiplier;
  ...
  const tableDataMacro = {
    ...
    body: macroNutrients.map((nutrient) => mapNutrient(nutrient)),
  };

  const tableDataMicro = {
    head: ['Nutrition', 'Quantity', 'DV'],
    body: microNutrients.map((nutrient) => mapNutrient(nutrient)),
  };
